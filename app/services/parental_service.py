...
from app.models.models import User, ParentChild, ParentChildConnection, BlockedSite, DailySummary
...
def verify_parent_child_access(db: Session, parent_id: str, child_id: str) -> bool:
    ...
    # OTP verified access
    otp_link = db.query(ParentChildConnection).filter(
        ParentChildConnection.parent_id == parent_id,
    ...
    expires_at = _as_utc(otp_link.expires_at)
    now = datetime.now(timezone.utc)
    if expires_at and expires_at <= now:
        otp_link.status = "expired"
        db.commit()
        return False
